...
class TestSomething(unittest.TestCase):
    ...
    def test_sum_string_of_ints(self, mock_input):
        get_doc = get_doc_shelf()
        owner_name = get_doc_owner_name()
        # Проверка функции get_doc_shelf - на какой полке находится документ
        self.assertEqual(get_doc, '1')
        # Проверка функции get_doc_owner_name - кто владелец документа
        self.assertEqual(owner_name, 'Геннадий Покемонов')

    # Проверка функции get_all_doc_owners_names - список владельцев документов
    def test_get_all_doc_owners_names(self):
        all_doc_owners = get_all_doc_owners_names()
        self.assertEqual(all_doc_owners, {'Геннадий Покемонов', 'Василий Гупкин', 'Аристарх Павлов'})

    # Проверка функции add_new_shelf – команда добавит новую полку, если она отсутвует в списке
    @patch('builtins.input', return_value='777')
    def test_add_new_shelf(self, mock_input):
        add_new_shel_test = add_new_shelf()
        self.assertEqual(add_new_shel_test, ('777', True))
